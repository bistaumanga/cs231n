...
def conv_backward_naive(dout, cache):
  ...
  dw = np.zeros_like(w)
  dx = np.zeros_like(x)
  dx_pad = np.pad(dx, ((0,), (0,), (P,), (P,)), 'constant')
  db = np.zeros_like(b)
  ...
  for n in range(N):
    for f in range(F):
      for i in range(H_):
        for j in range(W_):
          x_win = x_pad[n, :, i * S : i * S + HH, j * S : j * S + WW]
          dw[f] += x_win * dout[n, f, i, j]
...
def max_pool_forward_naive(x, pool_param):
  ...
  for n in xrange(N):
    for c in xrange(C):
      for i in xrange(H_):
        for j in xrange(W_):
          x_win = x[n, c, i * S : i * S + HH, j * S : j * S + WW]
    ...
    for c in xrange(C):
      for i in xrange(H_):
        for j in xrange(W_):
          x_win = x[n, c, i * S : i * S + HH, j * S : j * S + WW]
          x_max = np.max(x_win)
          dx[n, c, i * S : i * S + HH, j * S : j * S + WW]+=(x_win == x_max) * dout[n, c, i, j]
